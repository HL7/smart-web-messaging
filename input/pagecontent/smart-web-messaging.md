...
| `additionalResponsesExpected` | OPTIONAL     | boolean | If `true`, indicates that the recipient can expect additional responses to arrive with the same `responseToMessageId`. If `false` or if this property is omitted, the recipent can expect that no additional responses will arrive. |
