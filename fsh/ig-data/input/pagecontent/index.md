...
should be readily supportable in browser controls for any HTML5-capable
browser-based EHR.
...
| Property              | Optionality  | Type   | Description |
| --------------------- | ------------ | ------ | ----------- |
| `messageId`           | REQUIRED     | ID     | A unique ID for this message generated by the caller. |
| `responseToMessageId` | REQUIRED     | ID     | The `messageId` of the received message that this message is in response to. |
| `responsesRemainingNum`  | OPTIONAL     | INT    | If more than one responses will be sent, specifies the number of remaining responses.  A value of -1 indicates that the number of remaining responses is unspecified or unknown. |
